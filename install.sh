...
if [ "$INTERNAL_MACHINE" -eq 0 ]; then
    ...
    if [ "$UBUNTU_VER" != "16.04" ]; then
	cmake_install_with_option "trap-fpe" "-DTRAP_FPE_BLACKLIST=$WORKSPACE/drcutil/trap-fpe.blacklist.ubuntu1404"
    else
	cmake_install_with_option "trap-fpe" "-DTRAP_FPE_BLACKLIST=$WORKSPACE/drcutil/trap-fpe.blacklist.ubuntu1604"
    fi
else
