...
python ${WORKSPACE}/drcutil/getRobotPos.py | tee ${WORKSPACE}/${TASK}-getRobotPos.txt
RESULT=$(cat ${WORKSPACE}/${TASK}-getRobotPos.txt | python ${WORKSPACE}/drcutil/${TASK}-checkRobotPos.py)
echo "RESULT: ${RESULT}"
if [ "${TARGET}" != "" ]; then
  python ${WORKSPACE}/drcutil/getTargetPos.py ${TARGET} ${PORT} | tee ${WORKSPACE}/${TASK}-getTargetPos.txt
  RESULT=$(cat ${WORKSPACE}/${TASK}-getTargetPos.txt | python ${WORKSPACE}/drcutil/${TASK}-checkTargetPos.py ${VR})
  echo "RESULT: ${RESULT}"
fi
